...
## 一. 需求描述
  ...
  为网站添加 实时搜索功能的方案, 采用 Docusaurus官方支持 [Algolia DocSearch方案](https://docsearch.algolia.com/)

  *Docsearch* 每周一次爬取你的网站 (可以在网页界面上配置具体时间), 并将所有内容汇总到一个 *Algolia* 索引中

  随后，前端会调用 *Algolia API* 来直接查询这些内容
...
* *docsearch-config.json* (爬虫配置文件)
  ...
  > 需要修改3处: *index_name*, *start_urls* 和 *sitemap_urls*
...
> 以下两种 爬虫方式任选其一即可 (推荐使用 *GitHub Action*)
...
* 本地执行 爬虫脚本, 报错: *zsh: command not found: jq*
  ...
  > 确保清理本地 npm, yarn, pnpm的全局依赖中没有 *jq*
